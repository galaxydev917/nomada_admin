    ...
    $('body').on('click', '.updateData_employee', function () {
        ...
        firebase.database().ref('employee/' + updateID).on('value', function (snapshot) {
            ...
            <div class="form-group">\
		        <label for="status" class="col-md-12 col-form-label">Status</label>\
		        <div class="col-md-12">\
                    <select class="form-control" name="status"><option value="Active" ' + (values.status == "Active") ? selected="selected" : ""; + '>Active</option><option value="Inactive" ' + (values.status == "Inactive") ? selected="selected" : ""; + '>In-Active</option></select>\
		        </div>\
		    </div>';
